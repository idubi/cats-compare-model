...
def get_classified_box (yolo_detection_result) : 
    ...
    return {'category' : label, 'confidence' : confidence , 'box' : box}
    ...
    try:
        ...
        for result in results:
            ...
            #         return  # Skip non-cat/dog images
                ...
                image_save_path = os.path.join(OUTPUT_DIR, category, detection_quality_indicator_path, "images",image_name)
                ...
                label_save_path = os.path.join(OUTPUT_DIR, category, detection_quality_indicator_path, "labels",image_name.replace(".jpg", ".txt"))
                with open(label_save_path, "w") as f:
                    f.write(yolo_label)

                logger.info(f"✅ Processed {image_name}: Saved image & label , saved as {category}.{detection_quality_indicator_path}  confidence {confidence}  ")
